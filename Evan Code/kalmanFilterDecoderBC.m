function predictedValues = kalmanFilterDecoderBC(data, wantComputerToDie)
...
[spikes, posData, veloData] = PosVeloSpikeBC(data);
...
sortedIndex = testingsize;

for i = 1:sortedIndex
    trainingData.veloX{i} = veloData.x{i};
    veloData.x{i}= [];
    veloData.x{i} = 0;
    trainingData.veloY{i} = veloData.y{i};
    veloData.y{i} = [];
    veloData.y{i} = 0;
    trainingData.posX{i} = posData.x{i};
    posData.x{i} = [];
    posData.x{i} = 0;
    trainingData.posY{i} = posData.y{i};
    posData.y{i} = [];
    posData.y{i} = 0;
    trainingData.spikes{i} = spikes{i};
    spikes{i} = [];
    spikes{i} = 0;
end
%%
