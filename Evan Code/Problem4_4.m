...
[spikes,fiftyPos,fiftyVelo] = Problem4_0_1(Data);
%%
...
totalPreformancePosX = mean(abs(meanErrorPosX)) * 100;
totalPreformancePosY = mean(abs(meanErrorPosY)) * 100;
totalPreformanceVeloX = mean(abs(meanErrorVeloX)) * 100;
totalPreformanceVeloY = mean(abs(meanErrorVeloY)) * 100;
...
differencePosY = zeros(length(testingPosY),1);
...
for trial= 1:length(testingPosX)
         ...
         differencePosX(trial) = testingPosX{trial}(end) - predictedValues.muPos{trial}(end,1);
    
        
         differencePosY(trial) = testingPosY{trial}(end) - predictedValues.muPos{trial}(end,2);

end 


% Plot preformance as number of neurons
